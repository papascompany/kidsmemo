...
## 12. 2026-06-15 Public Entry / Auth / Admin Direction

현재 배포 URL인 `https://kidsmemo.vercel.app/`는 더 이상 로그인 후 대시보드로만 해석하면 안 된다.

제품 진입 구조를 다음과 같이 분리한다.

- `/`: 공개 서비스 랜딩
- `/login`: 로그인
- `/signup`: 간편가입
- `/signup/jumbokids`: 점보키즈 아이디 회원인증 설계 화면
- `/onboarding`: 기관 생성 또는 초대 코드 참여
- `/app`: 원장/선생님 기관 대시보드
- `/admin`: 점보키즈 운영 관리자 콘솔

이번 방향 전환의 이유:

- 신규 원장님/선생님이 서비스 취지와 가입 흐름을 먼저 이해해야 한다.
- 현재 대시보드는 기관 데이터가 포함된 로그인 후 업무 화면에 가깝다.
- 점보키즈 아이디 회원인증은 실제 API 연동 전이라도 가입 설계와 관리자 관리 항목에 포함되어야 한다.
- 관리자 화면에는 기관별 쿠폰, 할인코드, 점보키즈/고도몰 링크 세팅 기능이 필요하다.

관리자 콘솔의 초기 범위:

- 기관별 쿠폰/할인코드 등록
- 쿠폰명, 설명, 코드, 혜택 라벨, 유효기간, 대상 역할 관리
- 점보키즈 링크와 고도몰 링크 관리
- 점보키즈 인증 상태 추적
- 회원/권한 상태 확인
- 다운로드 이력 추적 준비
...
- 현재 `/app`는 mock/fallback 데모 접근을 유지한다.
- Supabase live 전환 전에는 실제 세션 기반 `requireUser`, `requireMembership`, `requireOrgRole`, `requirePlatformAdmin`이 필요하다.
- `admin`은 장기적으로 기관 멤버 역할이 아니라 플랫폼 운영자 권한으로 분리해야 한다.
- 일반 사용자 API는 service role client가 아니라 user-scoped session client를 사용해야 한다.
