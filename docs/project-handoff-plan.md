...
- 예정된 행사 전날, 점보키즈에서 즉시 사용 가능한 쿠폰/적립금 안내를 문자, 카카오 알림톡, 이메일로 자동 발송한다.
- 점보키즈는 사용자가 직접 운영 중인 포토북, 앨범, 사진 인화 온라인 서비스다.
- 쿠폰은 점보키즈 API로 자동 발급하거나, 관리자가 직접 등록한 수동 쿠폰 링크/코드와 안내 콘텐츠를 발송할 수 있다.
...
- 원장 계정은 마이페이지에서 자신의 기관 정보, 행사 일정, 쿠폰 캠페인, AI 행사 조언, 발송 설정을 관리한다.
- 원장/교사 계정은 자신이 속한 기관의 데이터만 볼 수 있어야 한다.
- 다른 유치원/어린이집의 일정, AI 결과, 쿠폰 설정, 학부모 발송 이력은 절대 노출되면 안 된다.
...
- 행사 일정, AI 조언, 쿠폰 캠페인은 서로 연결되어 있다는 느낌을 주되, 화면 조작은 단순해야 한다.
...
- 기관별 연간 행사 등록
- 행사 등록/수정
- 행사일, 대상 연령/반, 설명, 준비물, 연결 쿠폰 캠페인 관리
- 반복 행사 지원 예정
...
쿠폰 캠페인 생성 시 발행 방식을 선택한다.

- `jumbokids_api`: 점보키즈 API로 발송 직전 쿠폰/적립금 발급
- `manual`: 관리자가 직접 입력한 쿠폰 코드/링크를 발송
...
- 공용 코드/링크 방식이 기본
- 한 캠페인에 쿠폰 1개 또는 여러 개 등록 가능
- 여러 쿠폰은 하나의 안내 랜딩 페이지로 묶어 발송
- 안내 콘텐츠는 `html` 또는 `image`
- 이미지 안내는 향후 Supabase Storage 업로드로 연결 예정

### 자동 발송

- 매일 오전 배치 또는 cron으로 다음 날 행사를 조회
- 행사에 연결된 쿠폰 캠페인을 확인
- API 발급형이면 점보키즈 API를 호출
- 수동발행형이면 등록된 쿠폰 코드/링크와 안내 랜딩을 사용
- 발송 채널 우선순위:
  - 카카오 알림톡
  - SMS/LMS
  - 이메일
...
11. 쿠폰 캠페인/아이템/대상/안내 persistence 구현
12. 공개 쿠폰 랜딩 접근 정책 설계
13. Supabase Storage 이미지 업로드 구현
